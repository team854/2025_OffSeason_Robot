...
public class SwerveSubsystem extends SubsystemBase {
    ...
    private void createAndConfigureSwerve() {
        ...
        Pose2d startingPose = new Pose2d(new Translation2d(Constants.Starting.X.in(Meter), Constants.Starting.Y.in(Meter)), new Rotation2d()); 

        try {
            System.out.println("Initalizing swerveDrive");
            swerveDrive = new SwerveParser(new File(Filesystem.getDeployDirectory(), Constants.SwerveConstants.SWERVECONFIGDIR)).createSwerveDrive(Constants.RobotKinematicConstants.MAX_SPEED.in(MetersPerSecond), startingPose);
